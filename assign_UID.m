	...
	if exist('IMAGE_PID')
		...
		for i = 1:length(step)
			...
			if isempty(j) && isempty(k)			 % monotonic
				...
				if isrowcustom(ib_lo); ib_lo = ib_lo'; end;
				if isrowcustom(ib_hi); ib_hi = ib_hi'; end;
				ib = [ib_lo; ib_hi];
			end		
			% assign image UID
	...
	if exist('AIDA_PID')
		...
		for i = 1:length(step)		
			...
			if isempty(j) && isempty(k)			 % monotonic
				...
				if isrowcustom(ab_lo); ab_lo = ab_lo'; end;
				if isrowcustom(ab_hi); ab_hi = ab_hi'; end;
				ab = [ab_lo; ab_hi];
			end		
			% assign image UID
...
function out = isrowcustom(var)
	[xsize,ysize] = size(var);
	yisint = (round(ysize) == ysize);
	if ( xsize == 1 ) && ( yisint ) && ( ysize > 0 )
		out = true;
	else
		out = false;
	end
